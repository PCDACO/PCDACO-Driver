...
export const Calendar = ({ initialDate }: CalendarProps) => {
  ...
  const renderDay = ({ item }: { item: Date | null }) => {
    ...
    const isActive = item && selectedDate && item.toDateString() === selectedDate.toDateString();
  ...
  return (
    <View>
      <HStack center className="mb-6 justify-between">
        <Text className="font-tmedium -mb-2 text-lg text-primary">
          {`Tháng ${currentDate.getMonth() + 1}, ${currentDate.getFullYear()}`}
        </Text>

        <HStack gap={0}>
          <TouchableOpacity activeOpacity={0.7} className="p-2" onPress={handlePrevMonth}>
            <Feather name="chevron-right" size={24} color={COLORS.dark.primary} />
          </TouchableOpacity>
          <TouchableOpacity activeOpacity={0.7} className="p-2" onPress={handleNextMonth}>
      ...
      <HStack gap={0} className="mb-2 flex-row justify-between">
        {daysOfWeek.map((day, index) => (
          <Text key={index} className="font-tbold w-[14.28%] text-center text-base text-primary">
            {day}
          </Text>
        ))}
      </HStack>
